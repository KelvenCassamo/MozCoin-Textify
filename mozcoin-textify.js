...
if(response != undefined){
          ...
          var n = parseFloat(this.replace(",", `${responseData.separadorDecimal}`), 10).toFixed(2).toString().split(`${responseData.separadorDecimal}`);
          ...
          //Pegar a parte inteira.
           ...
           function extensoValor(valor) {
               ...
               var a, n, v, i, n = valor.toString().split("."), e = ` ${responseData.conjuncao} `, $ = `${responseData.unitsingular}`, d = `${responseData.subunitsingular}`, sl;
               var r = [], s = [], t = "";
               ...
               for (var f = n.length - 1, l, j = -1; ++j <= f; s = []) {   
                   ...
                   j && (n[j] = ((`${responseData.separadorDecimal}` + n[j]) * 1).toFixed(2).slice(2));
           ...
           var result = extensoValor(meticais) + " " + (meticais === 1 ? `${responseData.unitsingular}` : `${responseData.unitplural}`);
           if (centavos > 0) {
               result += ` ${responseData.conjuncao} ` + extensoValor(centavos) + " " + (centavos === 1 ? `${responseData.subunitsingular}` : `${responseData.subunitplural}`);
           }
       ...
       resolve("");
         ...
         reject("Ocorreu um erro ao ler o arquivo config.json");
     }      
